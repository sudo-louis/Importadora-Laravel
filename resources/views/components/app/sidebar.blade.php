@php
    ...
    // Plantillas pertenece a Reportes (y evita RouteNotFoundException)
    $hasPlantillasRoute = \Illuminate\Support\Facades\Route::has('plantillas.index');
    $canPlantillas = $canReportes && $hasPlantillasRoute;

    $reportesOpen = in_array($current, ['reportes.index','plantillas.index'], true);
    $actividadOpen = in_array($current, ['actividad.contenedores','actividad.usuarios'], true);
@endphp
    ...
    <nav class="flex-1 px-3 py-4 space-y-2 overflow-y-auto">
        ...
        @if($canContenedores)
            <a href="{{ route('contenedores.index') }}"
               class="flex items-center gap-3 px-4 py-3 rounded-2xl transition
                      {{ str_starts_with((string)$current, 'contenedores.') ? 'bg-blue-50 text-blue-700 dark:bg-slate-800 dark:text-white' : 'text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-slate-800' }}">
        ...
        {{-- Reportes + Plantillas --}}
        @if($canReportes)
            <div x-data="{ open: {{ $reportesOpen ? 'true' : 'false' }} }" class="space-y-2">
                <button type="button" @click="open = !open"
                        class="w-full flex items-center justify-between px-4 py-3 rounded-2xl transition
                               {{ $reportesOpen ? 'bg-blue-50 text-blue-700 dark:bg-slate-800 dark:text-white' : 'text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-slate-800' }}">
                    <div class="flex items-center gap-3">
        ...
        @if($canUsuarios)
            <a href="{{ route('usuarios.index') }}"
               class="flex items-center gap-3 px-4 py-3 rounded-2xl transition
                      {{ str_starts_with((string)$current, 'usuarios.') ? 'bg-blue-50 text-blue-700 dark:bg-slate-800 dark:text-white' : 'text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-slate-800' }}">
        ...
        {{-- Actividad --}}
        @if($canActividad)
            <div x-data="{ open: {{ $actividadOpen ? 'true' : 'false' }} }" class="space-y-2">
                <button type="button" @click="open = !open"
                        class="w-full flex items-center justify-between px-4 py-3 rounded-2xl transition
                               {{ $actividadOpen ? 'bg-blue-50 text-blue-700 dark:bg-slate-800 dark:text-white' : 'text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-slate-800' }}">
                    <div class="flex items-center gap-3">
